...
module.exports = async (req, res, next) => {
  try {
    ...
    if (!UserSessionID) {
      res.OutgoingParams = {
        status: 400,
        key: API_ERROR.SESSION_NOT_FOUND,
        message: `Session not found.`,
      };
      return sendResponse(req, res, next);
    }
    // Find user session with given user session id (extracted from token)
    const userSession = await services.findOne(
      { where: { UserSessionID: UserSessionID, Status: "active" }, raw: false },
      "UserSession"
    );
    if (!userSession) {
      res.OutgoingParams = {
    ...
    if (
      commonUtils.getUnixTimestamp(userSession?.ExpiresAt) <=
      commonUtils.getUnixTimestamp(new Date())
    ) {
      await userSession?.update({ Status: "inactive" });
      res.OutgoingParams = {
        status: 400,
        key: API_ERROR.SESSION_EXPIRED,
    ...
    if (
      ...
      await userSession?.update({
        ExpiresAt: commonUtils.extendTime(
          new Date(),
          userSession?.TokenExtendTime,
          "h"
        ),
      });
    }
    ...
    const accessAllCustomers = permissionUtils.hasPermission(
      "customers_list",
      Permissions
    );

    const customer = await services.findOne(
      { where: { CustomerID: userSession?.CustomerID }, raw: false },
      "Customer"
    );
    if (
      !accessAllCustomers &&
      (!customer || customer?.Deleted || customer?.Status === "inactive")
    ) {
      await userSession?.update({ Status: "inactive" });
      res.OutgoingParams = {
        status: 400,
    ...
    res.OutgoingParams = {
      status: 403,
      key: API_ERROR.JWT_VERIFICATION_FAILED,
