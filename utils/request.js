...
module.exports = (req, res, next) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    if (req.IncomingParams.Password) {
      req.IncomingParams.Password = req?.IncomingParams.Password.replace(
        /[\s\0\x0B\xc2\xa0]/g,
        ""
      );
      forLogs.Password = "***************";
    ...
    if (req.IncomingParams.NewPassword) {
      req.IncomingParams.NewPassword = req?.IncomingParams.NewPassword.replace(
        /[\s\0\x0B\xc2\xa0]/g,
        ""
      );
      forLogs.NewPassword = "***************";
    ...
    if (req.IncomingParams.OldPassword) {
      req.IncomingParams.OldPassword = req?.IncomingParams.OldPassword.replace(
        /[\s\0\x0B\xc2\xa0]/g,
        ""
      );
      forLogs.OldPassword = "***************";
